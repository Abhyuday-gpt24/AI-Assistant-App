...
export function Chatbox({
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-border bg-card p-2 shadow-sm focus-within:border-ring focus-within:ring-2 focus-within:ring-(--ring)/30">
        {attachments.length > 0 && (
          <ul className="mb-2 flex flex-wrap gap-2 px-1">
            {attachments.map((file) => (
              <li
                key={file.id}
                className="flex max-w-55 items-center gap-2 rounded-lg border border-border bg-muted px-2 py-1 text-xs text-foreground"
              >
                {file.status === "uploading" ? (
                  <SpinnerIcon className="h-3.5 w-3.5 shrink-0 animate-spin text-muted-foreground" />
                ) : (
                  <FileIcon
                    className={
                      file.status === "error"
                ...
                {file.status === "uploaded" && file.ingestStatus === "ready" && (
                  <span className="shrink-0 text-muted-foreground">indexed</span>
                )}
                ...
                <button
                  ...
                  className="ml-0.5 inline-flex h-4 w-4 shrink-0 items-center justify-center rounded text-muted-foreground hover:text-foreground"
                >
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            title="Attach file"
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-40"
          >
      ...
      <p className="mt-2 text-center text-[11px] text-muted-foreground">
        ...
        to send, <kbd className="rounded border border-border px-1">Shift</kbd>+
        <kbd className="rounded border border-border px-1">Enter</kbd> for new line.
      </p>
    </form>
  );
}

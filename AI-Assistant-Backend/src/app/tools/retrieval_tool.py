...
def retrieve_tool(state: AgentState)-> AgentState:
    # Retrieval is scoped to this turn's RAG namespace: the project's shared
    # corpus for a project chat, else the chat's own isolated namespace (the api
    # layer resolves which — see services/namespace.py). Falls back to chat_id for
    # older state shapes. With no namespace there's no KB to search → empty result.
    namespace = state.get("rag_namespace") or state.get("chat_id")
    if not namespace:

...
clearing all of them:
  ...
  1. S3        — the whole '{user_id}/{chat_id}/' prefix (source files, converted
                 Markdown, extracted images all live there).
  2. Pinecone  — the chat's vectors in the tenant namespace, deleted by the
                 '{chat_id}#' id-prefix.
...
from src.api.services.s3_bucket_service import delete_chat_files_from_s3
from src.api.services.chat_service import delete_chat_thread
from src.app.rag_pipeline.vector_store import delete_chat_vectors
...
async def delete_chat(chat_id: str, user_id: str, db: AsyncSession, s3) -> dict:
    ...
    `user_id`.
    """
    # Gather the chat's doc paths (for the ingestion-job cleanup) before wiping.
    res = await db.execute(
        select(ChatAttachment.storage_path).where(ChatAttachment.chat_id == chat_id)
    )
    doc_paths = list(res.scalars().all())

    # 1) S3 — one prefix sweep removes source + markdown + extracted (best-effort).
    try:
        result = await asyncio.to_thread(
            delete_chat_files_from_s3, user_id, chat_id, s3
        )
    ...
    # 2) Pinecone — the chat's vectors in the tenant namespace (best-effort).
    await asyncio.to_thread(delete_chat_vectors, chat_id)

    # 3) LangGraph checkpointer thread (best-effort).
    await delete_chat_thread(chat_id)

    # 4) DB rows — last, and transactional.

"""Per-chat document ingestion: S3 source doc → Markdown → chunks → Pinecone.
...
worker that the upload flow triggers; it only needs an S3 client and the shared
tenant vector store.

Every chunk lands in the ONE tenant namespace; per-chat isolation is carried in
the chunk metadata (`chat_id`) and the vector-id prefix (`{chat_id}#…`), so a
chat only ever retrieves / deletes its own vectors.
...
Pipeline per document:
  ...
  4. save the Markdown to S3 ('{user_id}/{chat_id}/markdown/{file_id}.md')
  5. chunk the Markdown and embed into the tenant namespace, tagged with
...
from src.app.rag_pipeline.data_ingestion.utils.chunk_metadata import build_chunk_documents
from src.app.rag_pipeline.vector_store import get_vector_store, chunk_vector_id
...
def ingest_document(storage_path: str, original_name: str, content_type: str,
    ...
    """Convert one uploaded document and embed it into the tenant namespace,
    tagged so only this `chat_id` retrieves it.

    `user_id` / `chat_id` own the S3 paths (source, extracted images, saved
    Markdown all under '{user_id}/{chat_id}/…') and ride in every chunk's
    metadata. The single tenant namespace is implicit (see vector_store)."""
    ...
    for img in result["images"]:
        key = extracted_image_key(storage_path, img["name"])
    ...
    md_key = markdown_key(storage_path)
    upload_bytes(md_key, markdown.encode("utf-8"), "text/markdown", s3)

    # 5) chunk + embed into the tenant namespace with the per-chat metadata schema
    chunks = chunk_markdown(markdown, source=original_name, storage_path=storage_path)
    if chunks:
        store = get_vector_store()
        documents = build_chunk_documents(
            chunks, user_id=user_id, chat_id=chat_id, source="user_upload",
        )
        ids = [chunk_vector_id(chat_id, c["chunk_id"]) for c in chunks]
        store.add_documents(documents, ids=ids)

    return {
        "storage_path": storage_path,
        "markdown_path": md_key,
        "file_id": file_id_for(storage_path),
        "chunks": len(chunks),
...
async def run_ingestion_jobs(files: list[dict], user_id: str, chat_id: str) -> None:
    ...
    isolated per file. `chat_id` tags every chunk so only this chat retrieves it
    (all chunks share the one tenant namespace).
    """

...
async def process_uploads(
    ...
    Every chunk is embedded into the single tenant namespace and tagged with
    `chat_id` so only this chat retrieves it. If the chat already exists it MUST
    belong to the caller (else 404, same gate as chat-stream; poisoning another
    user's chat is the attack case). If it doesn't exist yet (the frontend
    generates the id before the first message), the chat_id is used as-is.
    """
